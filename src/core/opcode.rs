use crate::{
    memory_map::Interconnect, memory_map::MemIdx, register::Condition, register::Registers,
};

use crate::memory_map::{Imm16, Imm8, Mem, Mem16, SignedImm8};
...
impl Decoder {
    ...
    fn parse_cb(memory: &Interconnect, registers: &Registers, pc: u16) -> Opcode {
        let value = Mem(MemIdx::Direct(pc)).read(memory, registers);
        let location: Either<Reg8, Mem> = match value % 8 {
            ...
            6 => Either::Right(Mem(MemIdx::Reg16(Reg16::HL))),
    ...
    /// it.
    pub fn decode(registers: &Registers, memory: &Interconnect) -> Opcode {
        Decoder::decode_with_context(registers, memory, Reg16::PC.read(registers)).unwrap()
    }
    ...
    pub fn decode_with_context(
        ...
        let dst: Either<Reg8, Mem> = match (value / 8) % 8 {
            0 => Either::Left(Reg8::B),
            1 => Either::Left(Reg8::C),
            2 => Either::Left(Reg8::D),
            3 => Either::Left(Reg8::E),
        ...
        let src: Either<Reg8, Mem> = match value % 8 {
            0 => Either::Left(Reg8::B),
            1 => Either::Left(Reg8::C),
            2 => Either::Left(Reg8::D),
            3 => Either::Left(Reg8::E),
        ...
        Some(match value {
            0x00 => Opcode::NOP,
            0x10 => Opcode::STOP,
            0x20 => Opcode::JR(Some(Condition::NZ)),
            0x30 => Opcode::JR(Some(Condition::NC)),
            0x01 | 0x11 | 0x21 | 0x31 => Opcode::LD16(reg16_arithmetic, Imm16),
            0x02 | 0x12 => Opcode::LDRegMemA(Mem(MemIdx::Reg16(reg16_arithmetic)), Reg8::A),
            0x22 => Opcode::LDHLMemIncA(Mem(MemIdx::Reg16(Reg16::HL)), Reg8::A),
            0x32 => Opcode::LDHLMemDecA(Mem(MemIdx::Reg16(Reg16::HL)), Reg8::A),
            0x03 | 0x13 | 0x23 | 0x33 => Opcode::INC16(reg16_arithmetic),
            0x04 | 0x14 | 0x24 => Opcode::INC(dst.left().unwrap()),
            0x34 => Opcode::INCHLMem(Mem(MemIdx::Reg16(Reg16::HL))),
            0x05 | 0x15 | 0x25 => Opcode::DEC(dst.left().unwrap()),
            0x35 => Opcode::DECHLMem(Mem(MemIdx::Reg16(Reg16::HL))),
            0x06 | 0x16 | 0x26 => Opcode::LDRegImm(dst.left().unwrap(), Imm8),
            0x36 => Opcode::LDHLMemImm(Mem(MemIdx::Reg16(Reg16::HL)), Imm8),
            0x07 => Opcode::RLCA,
            0x17 => Opcode::RLA,
            0x27 => Opcode::DAA,
            0x37 => Opcode::SCF,
            0x08 => Opcode::LDImmMemSP(Mem16(MemIdx::Imm16), Reg16::SP),
            0x18 => Opcode::JR(None),
            0x28 => Opcode::JR(Some(Condition::Z)),
            0x38 => Opcode::JR(Some(Condition::C)),
            0x09 | 0x19 | 0x29 | 0x39 => Opcode::ADDHL(Reg16::HL, reg16_arithmetic),
            0x0A | 0x1A => Opcode::LDARegMem(Reg8::A, Mem(MemIdx::Reg16(reg16_arithmetic))),
            0x2A => Opcode::LDAHLMemInc(Reg8::A, Mem(MemIdx::Reg16(Reg16::HL))),
            0x3A => Opcode::LDAHLMemDec(Reg8::A, Mem(MemIdx::Reg16(Reg16::HL))),
            ...
            0xE0 => Opcode::LDHImmMemA(Mem(MemIdx::Imm8), Reg8::A),
            0xF0 => Opcode::LDHAImmMem(Reg8::A, Mem(MemIdx::Imm8)),
            0xC1 | 0xD1 | 0xE1 | 0xF1 => Opcode::POP(reg16_stack),
            0xC2 => Opcode::JP(Some(Condition::NZ), Imm16),
            0xD2 => Opcode::JP(Some(Condition::NC), Imm16),
            0xE2 => Opcode::LDHCMemA(Mem(MemIdx::Reg8(Reg8::C)), Reg8::A),
            0xF2 => Opcode::LDHACMem(Reg8::A, Mem(MemIdx::Reg8(Reg8::C))),
            ...
            0xEA => Opcode::LDImmMemA(Mem(MemIdx::Imm16), Reg8::A),
            0xFA => Opcode::LDAImmMem(Reg8::A, Mem(MemIdx::Imm16)),
            0xCB => Decoder::parse_cb(memory, registers, pc + 1),
...
pub enum Opcode {
    ...
    // Loads (Blue)
    LDARegMem(Reg8, Mem),
    LDHLMemIncA(Mem, Reg8),
    LDHLMemDecA(Mem, Reg8),
    LDRegMemA(Mem, Reg8),
    LDAHLMemInc(Reg8, Mem),
    LDAHLMemDec(Reg8, Mem),
    LDRegImm(Reg8, Imm8),
    LDHLMemImm(Mem, Imm8),
    LD(Reg8, Reg8),
    LDRegHLMem(Reg8, Mem),
...
impl Opcode {
    ...
    pub fn try_display(
        ...
        let pc = pc.unwrap_or_else(|| Reg16::PC.read(registers));

        let signed_imm8 = Mem(MemIdx::Direct(pc + 1)).read(memory, registers) as i8;
        let imm8 = Mem(MemIdx::Direct(pc + 1)).read(memory, registers);
        let imm16 = Mem16(MemIdx::Direct(pc + 1)).read(memory, registers);
        ...
        let result = match self {
            ...
            Opcode::JP(c, _) => match c {
                Some(c) => format!("JP {}, x{:04x}", c, imm16),
                None => format!("JP x{:04x}", imm16),
            },
            Opcode::CALL(c, _) => match c {
                Some(c) => format!("CALL {}, x{:04x}", c, imm16),
                None => format!("CALL x{:04x}", imm16),
            },
            ...
            Opcode::LDHLSPOffset => format!("LD HL, SP+0x{:02x}", signed_imm8),
    ...
    pub fn display(&self, registers: &Registers, memory: &Interconnect, pc: Option<u16>) -> String {
        match self.try_display(registers, memory, pc) {
            Ok(s) => s,
            Err(s) => s,
        }
    }
